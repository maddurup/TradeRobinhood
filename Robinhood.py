...
class Robinhood:
    ...
    def place_order(
        ...
        # res.raise_for_status()
        return res
    # self.place_order_sl(instrument, quantity, bid_price, transaction, trigger='stop')
    def place_order_sl(
            self,
            instrument,
            quantity=1,
        ...
        if isinstance(transaction, str):
            transaction = Transaction(transaction)
        if not stop_price:
            stop_price = self.quote_data(instrument['symbol'])['stop_price']
        payload = {
            'account': self.get_account()['url'],
            'instrument': unquote(instrument['url']),
        ...
        #    instrument['symbol']
        #)
        print("=== Stop Loss Created as below === \n")
        pp.pprint(payload)
        res = self.session.post(
            self.endpoints['orders'],
            data=payload
    ...
    def place_stop_loss_order(
        ...
        Returns:
            (:obj:`requests.request`): result from `orders` put command
        """
        pp.pprint(self.cancel_open_orders_for(instrument['url']))
        transaction = Transaction.SELL
        return self.place_order_sl(instrument, quantity, bid_price, transaction, trigger='stop')
    ...
    def cancel_open_orders(self):
        ...
        for order in open_orders:
            ...
            cancelled_orders.append([order['side'],order['trigger'],order['quantity'], order['stop_price'], order['price'], order['type'] ])

        return cancelled_orders

    def cancel_open_orders_for(self, symbol_instrument):
        """
        Cancels all open orders and returns the responses from the canelled orders (results from `orders` command).
        If no orders are open or no orders were cancelled (i.e. failed to cancel), returns `None`
        """
        ...
        print("=== Cancelled Orders ===")
        for order in open_orders:
            # pp.pprint(order['instrument'])
            # pp.pprint(symbol_instrument)
            if order['instrument'] == symbol_instrument:
                self.cancel_order(order['id'])
                cancelled_orders.append([order['side'],order['trigger'],order['quantity'], order['stop_price'], order['price'], order['type'] ])
